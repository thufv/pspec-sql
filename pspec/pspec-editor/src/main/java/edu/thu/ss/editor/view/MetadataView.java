...
public class MetadataView extends EditorView<MetadataModel, XMLMetaRegistry> {
	...
	private void initializeLabel(Composite parent) {
		Group schemaGroup = EditorUtil.newInnerGroup(parent, "");
		schemaGroup.setLayout(new GridLayout(2, true));
		EditorUtil.newLabel(schemaGroup, getMessage(Metadata_Database), EditorUtil.labelData());
		databaseCombo = EditorUtil.newCombo(schemaGroup, null);
		EditorUtil.newLabel(schemaGroup, getMessage(Metadata_Table), EditorUtil.labelData());
		tableCombo = EditorUtil.newCombo(schemaGroup, null);
		...
		labelTree = new Tree(contentForm, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL);
	...
	public boolean saveTableLabeling() {
		...
		for (int i = 0; i < items.length; i++) {
			...
			for (int j = 0; j < subItems.length; j++) {
				...
				if (subItem.getText(1).isEmpty()) {
					table.onMetadataLabelError(MetadataLabelType.Label_Empty, databaseName + "." + tableName
							+ "." + subItem.getText(0));
					error = true;
				} else if (subItem.getText(2).isEmpty()) {
					table.onMetadataLabelError(MetadataLabelType.Extraction_Empty, databaseName + "."
							+ tableName + "." + subItem.getText(0));
					error = true;
				} else {
					String columnName = subItem.getText(0);
					String label = subItem.getText(1);
					String extractionName = subItem.getText(2);
					model.addColumnExtraction(databaseName, tableName, columnName, extractionName, label);
				}
			}
		}
