...
private[hive] abstract class HiveFunctionRegistry
  ...
  def lookupFunction(name: String, children: Seq[Expression]): Expression = {
    ...
    if (classOf[UDF].isAssignableFrom(functionInfo.getFunctionClass)) {
      ...
      HiveGenericUdf(name, functionClassName, children)
    } else if (classOf[AbstractGenericUDAFResolver].isAssignableFrom(functionInfo.getFunctionClass)) {
      HiveGenericUdaf(name, functionClassName, children)

    } else if (classOf[GenericUDTF].isAssignableFrom(functionInfo.getFunctionClass)) {
      HiveGenericUdtf(name, functionClassName, Nil, children)
    } else {
...
private[hive] case class HiveSimpleUdf(name: String, functionClassName: String, children: Seq[Expression])
  extends HiveUdf(name) with HiveInspectors {
...
private[hive] case class HiveGenericUdf(name: String, functionClassName: String, children: Seq[Expression])
  extends HiveUdf(name) with HiveInspectors {
...
private[hive] case class HiveGenericUdaf(name: String,
...
private[hive] case class HiveGenericUdtf(name: String,
  ...
  override def toLabel(child: Seq[Label]): Label = {
    Function(child, name, null);
  }
