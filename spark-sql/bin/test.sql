...
SELECT d_date
  FROM date_dim JOIN customer ON d_date_sk = c_first_sales_date_sk;
...
SELECT i_brand
  FROM (SELECT i_brand
          FROM item JOIN store_sales ON i_item_sk = ss_item_sk
        UNION ALL
        SELECT i_brand
          FROM item JOIN store_returns ON i_item_sk = sr_item_sk) x;
...
SELECT d_date
 ...
 WHERE CASE WHEN 1 > 0 THEN ss_sold_date_sk ELSE sr_returned_date_sk END =
          d_date_sk;
