...
class RangeResolver {
  ...
  def resolve(plan: Aggregate): Map[String, Range] = {

    return resolveRange(plan.child);
  }
  ...
  private def simplifyPredicate(expr: Expression, plan: LogicalPlan): Expression = {
    expr match {
      case binary: BinaryComparison => {
        if (support(binary.left.dataType) && resolveLiteral(binary.right) != null) {
          return binary;
        } else if (resolveLiteral(binary.left) != null && support(binary.right.dataType)) {
  ...
  private def resolvePredicate(expression: Expression, plan: LogicalPlan): (String, Range) = {
    expression match {
      ...
      case in: InSet => {
        ...
        val attrStr = attr.asInstanceOf[Attribute].name;
        val result =
