...
public class XMLMetaRegistryParser implements MetaParserConstant {
	...
	private PrimitiveType parsePrimitiveType(Node columnNode, String columnName) {
		...
		for (int i = 0; i < list.getLength(); i++) {
			...
			if (Ele_Desensitize_Operation.equals(name)) {
				...
				logger.error("Data category attribute and sub {} element cannot appear together in column: {}.", name,
						columnName);
				error = true;
			}
		}
	...
	private BaseType parseComplexType(Node columnNode, String columnName) {
		...
		if (type == null) {
			logger
					.error("one of data-category attribute or struct/map/array element should exist in column: {} ", columnName);
			error = true;
		}
		return type;
	}
