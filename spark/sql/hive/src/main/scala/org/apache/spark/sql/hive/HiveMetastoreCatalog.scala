  ...
  def createTable(
    ...
    synchronized {
      try client.createTable(tbl, allowExisting) catch {
        case e: org.apache.hadoop.hive.metastore.api.AlreadyExistsException if allowExisting => // Do nothing
  ...
  object ParquetConversions extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan = {
      ...
      val toBeReplaced = plan.collect {
        // Write path
        case InsertIntoTable(relation: MetastoreRelation, _, _, _) // Inserting into partitioned table is not supported in Parquet data source (yet).
        ...
        // Write path
        case InsertIntoHiveTable(relation: MetastoreRelation, _, _, _) // Inserting into partitioned table is not supported in Parquet data source (yet).
        if !relation.hiveQlTable.isPartitioned &&
          hive.convertMetastoreParquet &&
          hive.conf.parquetUseDataSourceApi &&
          relation.tableDesc.getSerdeClassName.toLowerCase.contains("parquet") =>
        ...
        // Read path
        case p @ PhysicalOperation(_, _, relation: MetastoreRelation) if hive.convertMetastoreParquet &&
          hive.conf.parquetUseDataSourceApi &&
          relation.tableDesc.getSerdeClassName.toLowerCase.contains("parquet") =>
          val parquetRelation = convertToParquetRelation(relation)
          val attributedRewrites = relation.output.zip(parquetRelation.output)
      ...
      plan.transformUp {
        ...
        case InsertIntoTable(r: MetastoreRelation, partition, child, overwrite) if relationMap.contains(r) => {
          val parquetRelation = relationMap(r)
          InsertIntoTable(parquetRelation, partition, child, overwrite)
        }
        case InsertIntoHiveTable(r: MetastoreRelation, partition, child, overwrite) if relationMap.contains(r) => {
          val parquetRelation = relationMap(r)
          InsertIntoTable(parquetRelation, partition, child, overwrite)
        }
...
private[hive] case class MetastoreRelation(databaseName: String, tableName: String, alias: Option[String])(val table: TTable, val partitions: Seq[TPartition])(@transient sqlContext: SQLContext)
  extends LeafNode {
  ...
  override def calculateLabels(): Policy = {
    ...
    if (meta == null) {
      output.foreach(attr => projectLabels.put(attr, Insensitive(databaseName, tableName, attr)));
      null;
    } else {
      output.foreach(attr => {
        val labelType = meta.lookup(databaseName, tableName, attr.name);
        if (labelType != null) {
          projectLabels.put(attr, DataLabel(labelType, databaseName, tableName, attr));
        } else {
          ...
          if (conds != null) {
            ...
            projectLabels.put(attr, condLabel);
          } else {
            projectLabels.put(attr, Insensitive(databaseName, tableName, attr));
          }
        }
      });
      meta.getPolicy;
